class Kadai2_2 {
	...
	static int nan_num = 9;
	static int r1 = 0;
	static int r2 = 0;
	static int countSub = 0;
	static int dispNum = 1;
	...
	static void masuSubs(int i, int j) {
		int r = 0;
		int count43 = 0;
		if(masu_2[i][j] == 1) {
			return;
		...
		while(true) {
			count43++;
			r = (int)(Math.random()*nan_num) + 1;
		      	masu[i][j] = r;
			if(tateyokoSearch(i, j) != 1 && blockSearch() == 0) {
				masu_2[i][j] = 1;
				break;
			}
			if(count43 == 10) {
	...
	//受け取った要素の場所にランダムな数字を入れる
	static int masuConf() {
		for(int i = 0; i < nan_num; i++) {
			for(int j = 0; j < nan_num; j++) {
				if(masu[i][j] == 0 || masu_2[i][j] == 0 || blockSearch() == 1 || tateyokoSearch(i, j) != 0) {
					return 1;
				}
			}
		}
	...
	//配列初期化
	static void disp() {
		System.out.println(nan_num + "✕" + nan_num + "のナンプレ  " + dispNum + "回目");
		for(int i = 0; i < nan_num; i++) {
			for(int j = 0; j < nan_num; j++) {
